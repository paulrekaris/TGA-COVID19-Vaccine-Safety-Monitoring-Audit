...
- [Verify the Findings (5 minutes)](analysis/output-assessment-evidence-matrix.xlsx) – Open the dataset, go to the Evidence Matrix sheet (19 rows, one per Plan output), filter column D (Status), and count. The Rating Criteria sheet defines each classification; the Summary sheet pre-computes headline statistics. Tool-agnostic (spreadsheet, R, Python). Full data paper: [SSRN 10.2139/ssrn.6610438](https://doi.org/10.2139/ssrn.6610438).
